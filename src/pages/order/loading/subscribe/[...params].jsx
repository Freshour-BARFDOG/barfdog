  ...
  useEffect(() => {
    (async () => {
      if (paymentSuccess == true) {
        await router.push(
          `/order/orderCompleted/subscribe/${orderIdx}/${customUid}/${price}/${merchantUid}/${name}`,
        );
      } else if (imp_success == 'false' || paymentSuccess === false) {
        if (error_msg?.includes('결제포기')) {
          const cancel = await postObjData(
            `/api/orders/${orderIdx}/subscribe/cancel`,
          );
          // console.log(cancel);
        } else {
          // 모바일 결제 실패
          const availableErrorMessage = errorMessage || error_msg;
          await failedSubscribePayment({
            orderId: orderIdx,
            error_msg: availableErrorMessage,
            error_code: null,
            redirect: false,
          });
        }
...
export async function getServerSideProps({ query, req }) {
  ...
  if (imp_success == 'true') {
    ...
    const paymentResult = await localApi
      .post(`/api/iamport/iamportSubscribe`, data)
      .then((res) => res)
    ...
    if (code === 0) {
      ...
      if (response.status === 'paid') {
        ...
        // validation - 결제 금액 검증
        const isValid = await validPaymentSSR(req, {
          orderId: orderId,
        ...
        if (!isValid) {
          ...
          // 주문 금액 검증 기능 - 자체 구현
          const fail = await invalidSuccessSubscribePaymentSSR(req, {
            orderId: orderId,
            data: data,
          });
          if (fail.status === 200) {
        ...
        errorMessage =
          ...
          '결제가 완료되었으나, 알 수 없는 이유로 결제완료 되지 않았습니다.';
        paymentSuccess = false;
      }
    } else {
      // 카드사 요청에 실패 (paymentResponse is null)
      paymentSuccess = false;
    }
