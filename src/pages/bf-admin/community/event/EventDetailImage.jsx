...
export default function EventDetailImage({ id, setFormValues, formErrors, setFormErrors }) {
  const maxImageCount = 5;
  const maxFileSize = 1000000; // 1BM
  ...
  const multipleImageFileChangeHandler = async (e) => {
    const postApiUrl = 'api/admin/events/image';
    ...
    try {
      if (curFiles.length === 1) {
        ...
        if(hasError) return;
        setIsLoading(true);
        const response = await uploadImage(file, setFormValues, id, setFormErrors, postApiUrl);
        setFileList((prevState) => [
          ...prevState,
          { file: file, filename: file.name, id: response.id },
        ]);
  ...
  return (
    <>
      <div className="inp_section multiImageBox">
        {fileList.length > 0 && (
          <div className="col-5 grid-box grid-column">
...
const uploadImage = async (file, setFormValues, id, setFormErrors, postApiUrl) => {
  let result = {};
  ...
  const imageLeakOrder = response.data.leakOrder;
  const isFailed = response.status === 200 || response.status === 201;

  setFormValues((prevState) => ({
    ...prevState,
    [id]: [...prevState[id], { id: imageId, leakOrder: imageLeakOrder }],
  }));
  setFormErrors((prevState) => ({
    ...prevState,
  ...
  result.id = imageId;
  result.leakOrder = imageLeakOrder;
