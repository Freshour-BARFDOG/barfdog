...
export class URLPathClass {
  ...
  private saveAdminPath(curPath: string) {
    ...
    const jsonArr = this.jsonStringifiedArray(
      curPath,
      maxCount,
      storedAdminPathArr,
    );
  ...
  getStoredAdminPathArr(): [] {
    ...
    return storedStringData && storedStringData !== 'undefined'
      ? JSON.parse(storedStringData)
      : [];
  }
  ...
  private validAdminPublicPath(curPath: string): boolean {
    ...
    const ADMIN_PUBLIC_PATH_LIST: string[] = [
      '/index',
      '/login',
      '/login/resetPassword',
    ];
  ...
  checkAuthAndRedirect(usertype: string) {
    if (this.VALIDATION.MEMBER_PATH && usertype === USERTYPE.NON_MEMBER) {
      ...
      this.VALIDATION.ADMIN_PATH &&
      !this.VALIDATION.ADMIN_PUBLIC_PATH &&
