...
export const SubscribeItems = ({ itemList }) => {
  ...
  const onCancelSubscribeOrder = useCallback(
    async (item) => {
  ...
  const onDeleteBillingKey = async ({ customerUid, orderId }) => {
    if (!customerUid)
      return alert('구독해지 불가능한 주문입니다. 관리자에게 문의하세요.');
        ...
        {itemList?.length > 0 &&
          itemList.map((item, index) => (
            <li key={`subscribe-item-${index}`}>
              <div className={s.day}>
                {transformDate(item.subscribeOrderDto.orderDate, 'total')}
              </div>
              <div className={s.content_body}>
                  ...
                  <div className={s.flex_box}>
                    <div className={s.text}>
                      ...
                      <div className={s.last_text}>
                        ...
                        {item.subscribeOrderDto.subscribeCount}회차)
                      </div>
                    </div>
                    ...
                    <div className={s.text2}>
                      ...
                      <div>
                        {transformLocalCurrency(
                          item.subscribeOrderDto.paymentPrice,
                        )}
                        원
                      </div>
                    </div>
                ...
                <div className={s.right_box}>
                  {item.subscribeOrderDto.needToCancelNaverpaySubscribe && (
                    <button
                      className={`${s.btn} ${s['naverPay-btn']}`}
                      onClick={onDeleteBillingKey.bind(null, {
                        customerUid: item.subscribeOrderDto.customerUid,
                        orderId: item.subscribeOrderDto.orderId,
                      })}
                    >
                      {isLoading.deleteBillingKey[
                        item.subscribeOrderDto.orderId
                      ] ? (
                        <Spinner style={{ color: '#fff' }} />
                      ) : (
                        <span>네이버페이 구독 해지</span>
                      )}
                    </button>
                  )}
                  {item.subscribeOrderDto.orderStatus ===
                  orderStatus.BEFORE_PAYMENT ? (
                    <button
                      className={`${s.btn} ${s.cancelOrder}`}
                      onClick={onCancelSubscribeOrder.bind(null, item)}
                    >
