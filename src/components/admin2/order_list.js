...
const statusEngToKor = {
  ALL: '전체',
  BEFORE_PAYMENT: '결제전',
  CANCEL_PAYMENT: '결제취소', // 결제전, 결제취소(카드정보 작성중 취소 누른경우)
  CANCEL_RESERVED_PAYMENT: '예약된주문결제취소', // 예약된 주문 결제취소
  HOLD: '보류HOLD',
  RESERVED_PAYMENT: '예약결제', // 보류, 예약결제(아임포트 스케쥴걸린상태)
  FAILED: '최초결제실패',
  FAILED_RESERVED_PAYMENT: '예약된정기구독결제실패', // 최초 결제 실패(카드결제 실패), 예약된 정기구독 결제 실패
  PAYMENT_DONE: '결제완료', // 결제완료
  DELIVERY_BEFORE_COLLECTION: '배송확인',
  PRODUCING: '생산중',
  DELIVERY_READY: '배송준비', // 배송 확인, 생산중(구독주문), 배송준비(일반주문)
  DELIVERY_START: '배송출발', // 배송 출발
  DELIVERY_DONE: '배송도착완료', // 배송 도착 완료
  CANCEL_REQUEST: '주문취소요청',
  CANCEL_DONE_SELLER: '판매자귀책취소완료',
  CANCEL_DONE_BUYER: '구매자귀책취소완료', // 주문 취소 요청, 판매자귀책 취소완료, 구매자귀책 취소완료
  RETURN_REQUEST: '주문반폼요청',
  RETURN_DONE_SELLER: '판매자귀책반품완료',
  RETURN_DONE_BUYER: '구매자귀책취반품료', // 주문 반품 요청, 판매자귀책 반품완료, 구매자귀책 반품완료
  EXCHANGE_REQUEST: '주문교환요청',
  EXCHANGE_DONE_SELLER: '판매자귀책교환완료',
  EXCHANGE_DONE_BUYER: '구매자귀책교환완료', // 주문 교환요청, 판매자귀책 교환완료, 구매자귀책 교환완료
  CONFIRM: '구매확정', // 구매확정
};
...
const columns = [
  ...
  {
    title: '개인정보수정일',
    dataIndex: 'memberModifiedDate',
    key: 'memberModifiedDate',
  },
  ...
  {
    title: '누적배송수',
    dataIndex: 'memberAccumulatedSubscribe',
    key: 'memberAccumulatedSubscribe',
  },
  {
    title: '누적 구독 배송수',
  ...
  {
    title: '다음 결제액',
    dataIndex: 'nextPaymentPrice',
    key: 'nextPaymentPrice',
  },
  {
    title: '첫 결제일',
    dataIndex: 'memberFirstPaymentDate',
    key: 'memberFirstPaymentDate',
  },
  {
    title: '누적 결제금액',
    dataIndex: 'memberAccumulatedamount',
    key: 'memberAccumulatedamount',
  },
...
const filterDataGeneral = (data, search) => {
  ...
  for (let i = data.length - 1; i >= 0; --i) {
    ...
    if (data_tmp.dtype == 'general') {
      const arr_street = [
        data_tmp.deliveryStreet,
        data_tmp.deliveryDetailAddress,
        data_tmp.deliveryZipcode,
      ];
  ...
  return defaultData.filter((item) => {
    ...
    if (orderState_result && grade_result && search_result) return item;
  });
};
...
const filterDataSubscribe = (data, search) => {
  ...
  for (let i = data.length - 1; i >= 0; --i) {
    ...
    if (data_tmp.dtype == 'subscribe') {
      ...
      let totalGramRecipe_seok_tmp =
        totalGramRecipe1_seok_tmp +
        totalGramRecipe2_seok_tmp +
        totalGramRecipe3_seok_tmp +
        totalGramRecipe4_seok_tmp;
        ...
        children2: [
          {
            ...
            gramRecipe1: gramRecipe1_tmp < 1e-12 ? null : gramRecipe1_tmp,
            ...
            totalGramRecipe2:
              totalGramRecipe2_tmp < 1e-12 ? null : totalGramRecipe2_tmp,
            totalGramRecipe3:
        ...
        children3: [
          {
            ...
            gramRecipe1:
              gramRecipe1_seok_tmp < 1e-12 ? null : gramRecipe1_seok_tmp,
            gramRecipe2:
              gramRecipe2_seok_tmp < 1e-12 ? null : gramRecipe2_seok_tmp,
            gramRecipe3:
              gramRecipe3_seok_tmp < 1e-12 ? null : gramRecipe3_seok_tmp,
            gramRecipe4:
              gramRecipe4_seok_tmp < 1e-12 ? null : gramRecipe4_seok_tmp,
            totalGramRecipe1:
              totalGramRecipe1_seok_tmp < 1e-12
                ? null
                : totalGramRecipe1_seok_tmp,
            totalGramRecipe2:
              totalGramRecipe2_seok_tmp < 1e-12
                ? null
                : totalGramRecipe2_seok_tmp,
            totalGramRecipe3:
              totalGramRecipe3_seok_tmp < 1e-12
                ? null
                : totalGramRecipe3_seok_tmp,
            totalGramRecipe4:
              totalGramRecipe4_seok_tmp < 1e-12
                ? null
                : totalGramRecipe4_seok_tmp,
          },
        ],
        children4: [
          {
            ...
            dogNeutralization:
              data_tmp.dogNeutralization === true ? 'YES' : 'NO',
          },
        ],
        children5: [
          {
  ...
  return defaultData.filter((item) => {
    ...
    subscribe_array.forEach((e) => {
      if (
        search.subscribeState.includes(e) &&
        !subscribe_result &&
        item.subscription
      ) {
    ...
    if (search.searchText) {
      if (
        ...
        search_result = item.orderMemberName.includes(search.searchText);
        // console.log('sub>>>item.orderMemberName>>>', item.orderMemberName);
      } else if (
        search.searchType.includes('orderDeliveryName') &&
        item.orderRecivedName
      ) {
...
const expandedRowRender = (record) => {
  ...
  const columns1 = [
    ...
    {
      title: '하루당 필요 칼로리',
      dataIndex: 'dogNeededCalorie',
      key: 'dogNeededCalorie',
    },
  ];
  const columns2 = [
    ...
    {
      title: '종류',
      dataIndex: 'typeCountYYL',
      key: 'typeCountYYL',
      width: '5%',
    },
    {
      title: '총량',
      dataIndex: 'totalGramRecipes',
      key: 'totalGramRecipes',
      width: '5%',
    },
    {
      title: '레시피1 개당g',
      dataIndex: 'gramRecipe1',
      key: 'gramRecipe1',
      width: '5%',
    },
    {
      title: '레시피2 개당g',
      dataIndex: 'gramRecipe2',
      key: 'gramRecipe2',
      width: '5%',
    },
    {
      title: '레시피3 개당g',
      dataIndex: 'gramRecipe3',
      key: 'gramRecipe3',
      width: '5%',
    },
    {
      title: '레시피4 개당g',
      dataIndex: 'gramRecipe4',
      key: 'gramRecipe4',
      width: '5%',
    },
    {
      title: '레시피1 총량',
      dataIndex: 'totalGramRecipe1',
      key: 'totalGramRecipe1',
      width: '5%',
    },
    {
      title: '레시피2 총량',
      dataIndex: 'totalGramRecipe2',
      key: 'totalGramRecipe2',
      width: '5%',
    },
    {
      title: '레시피3 총량',
      dataIndex: 'totalGramRecipe3',
      key: 'totalGramRecipe3',
      width: '5%',
    },
    {
      title: '레시피4 총량',
      dataIndex: 'totalGramRecipe4',
      key: 'totalGramRecipe4',
    },
  ];
  ...
  const columns4 = [
    ...
    {
      title: '견출생후 달',
      dataIndex: 'dogStartAgeMonth',
      key: 'dogStartAgeMonth',
      width: '5%',
    },
    ...
    {
      title: '중성화',
      dataIndex: 'dogNeutralization',
      key: 'dogNeutralization',
    },
  ];
  ...
  const columns5 = [
    {
      title: '먹을수없는음식',
      dataIndex: 'dogInedibleFood',
      key: 'dogInedibleFood',
      width: '5%',
    },
    {
      title: '먹을수없는기타',
      dataIndex: 'dogInedibleFoodEtc',
      key: 'dogInedibleFoodEtc',
      width: '5%',
    },
    {
      title: '견주의',
      dataIndex: 'dogCaution',
      key: 'dogCaution',
      width: '5%',
    },
    {
      title: '견활동레벨',
      dataIndex: 'dogActivityLevel',
      key: 'dogActivityLevel',
      width: '5%',
    },
    {
      title: '견간식수',
      dataIndex: 'dogSnackCountLevel',
      key: 'dogSnackCountLevel',
      width: '5%',
    },
    {
      title: '견주당산책수',
      dataIndex: 'dogWalkingCountPerWeek',
      key: 'dogWalkingCountPerWeek',
      width: '5%',
    },
    {
      title: '견산책당시간',
      dataIndex: 'dogWalkingTimePerOneTime',
      key: 'dogWalkingTimePerOneTime',
    },
  ];
  ...
  return (
    <div>
      <p>
        레시피1: STARTER PREMIUM, 레시피2: TURKEY & BEEF, 레시피3: DUCK & LAMB,
        레시피4: LAMB & BEEF
      </p>
      <Table
        bordered={true}
        columns={columns0}
        dataSource={record.children0}
        pagination={false}
      />
      <Table
        bordered={true}
      ...
      <Table
        bordered={true}
        columns={columns4}
        dataSource={record.children4}
        pagination={false}
      />
      <Table
        bordered={true}
        columns={columns5}
        dataSource={record.children5}
        pagination={false}
      />
    </div>
  );
