...
const SearchTerm = ({searchValue, setSearchValue, title, tooltip}) => {
  ...
  const diffDate = getDiffDateNumber( from, to );
  ...
  const onChangeTerm = useCallback( (e) => {
    ...
    const from = diffDate === 'all' ? global_searchDateType.oldestDate : getDiffDate( diffDateFromPast );
    ...
    <>
      <div className={s['search-row']}>
        ...
        <div className={`${s['inp-wrap']} ${s['term']}`}>
          <div >
            {global_searchDateType.BUTTONS.map( (btn, i) => <button
              key={`SearchTermButton-${i}`}
              onClick={onChangeTerm}
              data-value={btn.diffDate}
              className={`admin_btn line basic_l ${Math.abs( diffDate ) === btn.diffDate ? s.active : ''}`}
            >
              {btn.label}
            </button> )}
          </div>
