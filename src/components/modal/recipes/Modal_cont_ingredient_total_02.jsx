    ...
    <div data-modal-type="turkeyAndBeef">
      <div className={s.modal_text_box4}>
        <p className={s.text_row_1}>
          칠면조가슴살(칠레산), 칠면조 정육(칠레산), 소보섭살(풀먹은 호주 방목
          유기농), 소사태살(풀먹은 호주 방목 유기농), 칠면조목뼈(칠레산),
          소간(국내산), 소비장(국내산), 소신장(국내산), 단호박(국내산),
          당근(국내산), 양배추(국내산), 콜리플라워(국내산), 그린빈(벨기에),
          애호박(국내산), 케일(국내산), 사과(국내산), 녹색바나나(필리핀),
          난황(국내산), 표고버섯(국내산), 커티지치즈(국내산), 딸기(국내산),
          블루베리(미국산), 밀싹(국내산), 햄프씨드(USDA 유기농 인증 미국산),
          치아씨드(USDA 유기농 인증 미국산), 아마씨드(USDA 유기농 인증 미국산),
          대구간유(USDA 유기농 인증 미국산), 코코넛오일(USDA 유기농 인증
          미국산), 켈프(USDA 유기농 인증 미국산), 스피루리나(USDA 유기농 인증
          미국산), 실리엄허스크(USDA 유기농 인증 미국산), 난각분(국내산),
          요오드화소금(미국산), 비타민E(미국산), 망간(미국산), 아연(미국산),
