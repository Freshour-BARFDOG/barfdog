...
export const Modal_cont_ingredient_total_09 = () => {
  return (
    <div data-modal-type="lambAndBeef">
      <div className={s.modal_text_box4}>
        <p className={s.text_row_1}>
          캥거루 정육(호주산), 캥거루 갈비(호주산), 소간(국내산),
          소비장(국내산),
          단호박(국내산),당근(국내산),양배추(국내산),콜리플라워(국내산),그린빈(벨기에산),
          애호박(국내산), 케일(국내산), 사과(국내산), 녹색바나나(필리핀산),
          표고버섯(국내산), 고구마(국내산), 커티지치즈(국내산), 딸기(국내산),
          블루베리(미국산), 밀싹(국내산), 햄프씨드(USDA 유기농인증
          미국산),치아씨드(USDA 유기농인증 미국산),아마씨드(USDA 유기농인증
          미국산), 켈프(USDA 유기농인증 미국산), 난각분(국내산), 대구간유(USDA
          유기농인증 미국산), 코코넛오일(USDA 유기농인증 미국산), 강황(USDA
          유기농인증 미국산), 스피루리나(USDA 유기농인증 미국산),
