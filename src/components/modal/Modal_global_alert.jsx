...
export default function Modal_global_alert({
  message,
  onClick,
  background,
  ...props
  ...
  return (
    <>
      ...
      >
        <div
          className={s['body']}
          style={hasAlert ? { display: 'flex' } : style}
        >
          <header className={s['title-section']}>
            <pre className={`${s.text} ${s.only}`}>
              {message || modalContextMessage}
            </pre>
          </header>
