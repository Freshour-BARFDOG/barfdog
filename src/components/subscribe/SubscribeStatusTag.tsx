import {subscribeStatus, subscribeStatusEnum} from "@store/TYPE/subscribeStatus";
...
export const SubscribeStatusTag = ({status, subscribeCount}: Props): JSX.Element => {
  ...
  } else if (status === subscribeStatus.BEFORE_PAYMENT && subscribeCount > 0 ||
    status === subscribeStatus.SUBSCRIBE_PENDING) {
    UIstatus = subscribeStatusEnum.SUBSCRIBE_PENDING
  } else if (status === subscribeStatus.SUBSCRIBING ||
  ...
  return <i className={`${s.tag} ${s[UIstatus]}`}>{subscribeStatus.KOR[UIstatus]}</i>;
