  ...
  const oneMealGramsWithRecipeInfosWithTags = useMemo(
    () =>
      ...
      calcOneMealGramsWithRecipeInfo({
        ...
        allRecipeInfos: info.recipeInfoList.map((recipe) => ({
          id: recipe.id,
  ...
  const subsribePlanItems = [
    {
      ...
      bodyDescHTML: {
        row1: (
          <>
            하루에
            <span>&nbsp;{subscribePlanType.FULL.numberOfPacksPerDay}팩</span>
          </>
        ),
        row2: (
          <>
