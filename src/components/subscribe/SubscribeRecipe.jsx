    ...
    (async () => {
      ...
      try {
        ...
        for (const recipeId of recipeIdList) {
          ...
          const recommendRecipeInfo = dogId && await getRecommendRecipe(dogId);
          allRecipeData.isRecommendRecipe = allRecipeData.id === recommendRecipeInfo.recommendRecipeId;
    ...
    <>
            ...
            <div className={s.notice_row_3}>
              <div className={s.color_box}>
                ...
                <div className={s.color_box_row_2}>
                  ...
                  {/*)}*/}
                  <br />
                  <em>{subscribeInfo.recipe.nameList.join(', ')}</em> 레시피에는{' '}
      ...
      <div className={s.btn_box}>
        <button className={s.btn} onClick={onActiveConfirmModal}>
          {isLoading.submit ? <Spinner style={{ color: '#fff' }} /> : '변경 레시피 적용하기'}
        </button>
      </div>
      {activeConfirmModal && (
        <Modal_confirm
