...
export const valid_isEmpty = (
  value,
  option = { detail: false, message: '필수 항목입니다.' },
) => {
  let error = '';
  if (option.detail) {
...
export const valid_policyCheckbox = (obj, standardObjList = []) => {
  ...
  standardObjList.forEach(
    (list) => list.required && requiredKeyList.push(list.label),
  );
...
export const valid_URL = (value) => {
  ...
  const url = value;
  const regExp =
    /(http|https):\/\/((\w+)[.])+(asia|biz|cc|cn|com|de|eu|in|info|jobs|jp|kr|mobi|mx|name|net|nz|org|travel|tv|tw|uk|us|app)(\/(\w*))*$/i;
...
export const valid_isTheSameArray = (beforeArr1, beforeArr2) => {
  if (
    !beforeArr1 ||
    !beforeArr2 ||
    (beforeArr1?.length === 0 && beforeArr2?.length === 0)
  ) {
    return false;
  }
